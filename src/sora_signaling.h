...
class SoraSignaling : public std::enable_shared_from_this<SoraSignaling>,
 ...
 public:
  ~SoraSignaling() { RTC_LOG(LS_INFO) << "SoraSignaling::~SoraSignaling"; }
 ...
 private:
  ...
  void DoSendUpdate(const std::string& sdp, std::string type) {
    ...
    if (dc_ && using_datachannel_ && dc_->IsOpen("signaling")) {
      ...
      SendDataChannel("signaling", boost::json::serialize(m));
    } else if (ws_) {
      ws_->WriteText(
          boost::json::serialize(m),
          [self = shared_from_this()](boost::system::error_code, size_t) {});
 ...
 private:
              ...
              if (self->config_.simulcast &&
                ...
                for (auto v : encodings_json) {
                  ...
                  if (p.count("maxFramerate") != 0) {
                    params.max_framerate =
                        p["maxFramerate"].to_number<double>();
                  }
                  if (p.count("active") != 0) {
                ...
                self->SetEncodingParameters(mid,
          ...
          [self = shared_from_this(), type, answer_type]() {
            boost::asio::post(*self->config_.io_context, [self, type,
              ...
              SessionDescription::CreateAnswer(
                  self->pc_,
                  [self,
                   answer_type](webrtc::SessionDescriptionInterface* desc) {
                    std::string sdp;
                    desc->ToString(&sdp);
                    //self->manager_->SetParameters();
  ...
  void DoConnect() {
    ...
    if (connecting_wss_.empty()) {
      SendOnDisconnect(SoraSignalingErrorCode::INVALID_PARAMETER,
                       error_messages);
      return;
    }
 ...
 private:
  void SetEncodingParameters(
    ...
    for (auto transceiver : pc_->GetTransceivers()) {
      RTC_LOG(LS_INFO)
          << "transceiver mid=" << transceiver->mid().value_or("nullopt")
          << " direction="
          << webrtc::RtpTransceiverDirectionToString(transceiver->direction())
          << " current_direction="
    ...
    for (auto enc : encodings) {
      RTC_LOG(LS_INFO) << "SetEncodingParameters: rid=" << enc.rid
                       ...
                       << " max_framerate="
                       << (enc.max_framerate
                               ? std::to_string(*enc.max_framerate)
                               : std::string("nullopt"))
  ...
  webrtc::DataBuffer ConvertToDataBuffer(const std::string& label,
    ...
    bool compressed =
        compressed_labels_.find(label) != compressed_labels_.end();
 ...
 private:
  ...
  // 仕様に準拠した onnegotiationneed イベントを発生させるために使用され、オペレーションチェーンが空の場合にのみ発生する。
  // オブザーバは、イベントを発生させるタスク（例：Chromium: jump to main thread）をキューに入れる責任がある。
  // `event_id` を使って識別されるイベントは、PeerConnection::ShouldFireNegotiationNeededEvent() が true を返す場合のみ発生しなければなりません。
  void OnNegotiationNeededEvent(uint32_t event_id) override {}

  // Called any time the legacy IceConnectionState changes.
  //
  // ICEの状態は、標準から少し遅れていることに注意してください。
  // 最も顕著な違いは、30秒後ではなく15秒後に「failed」が発生することと、これは実際にはICEとDTLSの組み合わせの状態を表しているので、
  // ICEが成功する一方でDTLSが失敗すると「failed」になるかもしれないことです。
  //
  ...
  void OnIceCandidate(const webrtc::IceCandidateInterface* candidate) override {
    ...
    boost::asio::post(
        *config_.io_context, [self = shared_from_this(), m = std::move(m)]() {
          ...
          self->ws_->WriteText(boost::json::serialize(m),
                               [self](boost::system::error_code, size_t) {});
        });
  }
  ...
  // 注意: これは Plan B と Unified Plan の両方のセマンティクスで呼び出される。
  // Unified Plan のユーザーは OnTrack を選ぶべきで、OnAddTrack は後方互換性のためにのみ呼ばれます (そして OnTrack と全く同じ状況で呼ばれます)。
  void OnAddTrack(
      rtc::scoped_refptr<webrtc::RtpReceiverInterface> receiver,
      const std::vector<rtc::scoped_refptr<webrtc::MediaStreamInterface>>&
          streams) override {}

  // 信号がトランシーバーがリモートエンドポイントからメディアを受信することを示すとき、これは呼び出される。
  // これは、SetRemoteDescriptionの呼び出しの間に発生する。受信トラックは以下の方法でアクセスすることができる。
  // 受信トラックは `transceiver->receiver()->track()` によって、関連するストリームは
  // `transceiver->receiver()->streams()` によってアクセスすることができます。
  // 注：これは、Unified Planのセマンティクスが指定されている場合にのみ呼び出されます。
  // This behavior is specified in section 2.2.8.2.5 of the "Set the RTCSessionDescription" algorithm:
  // https://w3c.github.io/webrtc-pc/#set-description
  void OnTrack(rtc::scoped_refptr<webrtc::RtpTransceiverInterface> transceiver)
      override {}

  // 信号が、あるトラックではもうメディアを受信しないことを示すときに呼び出されます。
  // Plan Bのセマンティクスでは、指定された受信機がPeerConnectionから削除され、トラックはミュートされます。
  // Unified Plan のセマンティクスでは、レシーバは残りますが、トランシーバの方向が送信専用または非アクティブに変更されます。
  // https://w3c.github.io/webrtc-pc/#process-remote-track-removal
  // TODO(hbos,deadbeef): Make pure virtual when all subclasses implement it.
  void OnRemoveTrack(
      rtc::scoped_refptr<webrtc::RtpReceiverInterface> receiver) override {}

  // WebRTCによって興味深い使い方が検出されたときに呼び出されます。
  // 適切なアクションは、PeerConnection のコンテキストに関する情報を追加し、
  // 何らかの "interesting events" ログ関数にイベントを書き込むことです。
  // 何が「興味深い」のかを定義するためのヒューリスティックは、実装によって決まります。
              ...
              boost::asio::post(*self->config_.io_context, [self]() {
                ...
                SessionDescription::CreateAnswer(
                    ...
                    [self](webrtc::SessionDescriptionInterface* desc) {
                      ...
                      desc->ToString(&sdp);
                      boost::asio::post(
                          *self->config_.io_context, [self, sdp]() {
                            if (self->state_ != State::Connected) {
                              return;
                            }
