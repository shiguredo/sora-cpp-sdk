...
class FakeVideoCapturerImpl : public FakeVideoCapturer {
 public:
  FakeVideoCapturerImpl(FakeVideoCapturerConfig config)
      : FakeVideoCapturer(config), config_(config) {
    StartCapture();
  }
 ...
 private:
  void CaptureThread() {
    ...
    while (!stop_capture_) {
      ...
      if (result != BL_SUCCESS) {
        RTC_LOG(LS_ERROR) << "Failed to get image data from Blend2D: "
                          << result;
  ...
  void DrawDigitalClock(BLContext& ctx,
    ...
    // 時間（4桁）
    Draw7Segment(ctx, (hours / 1000) % 10, x, clock_y, digit_width,
                 digit_height);
    x += digit_width + spacing;
    Draw7Segment(ctx, (hours / 100) % 10, x, clock_y, digit_width,
                 digit_height);
    x += digit_width + spacing;
...
webrtc::scoped_refptr<FakeVideoCapturer> FakeVideoCapturer::Create(
    FakeVideoCapturerConfig config) {
  return webrtc::make_ref_counted<FakeVideoCapturerImpl>(std::move(config));
}
