...
2026-06-11 schedule の windows_x86_64 ジョブ (run [27318676246](https://github.com/shiguredo/sora-cpp-sdk/actions/runs/27318676246)、job id 80706826054) の failed log で `error 10051` の文字列を含む行は 7 件、`10051` の文字列を含む行は 15 件確認 (`gh run view --job=80706826054 --log-failed | grep -c "error 10051"` の出力。本ログは GitHub Actions の保持期限切れで再取得できないため、件数は 2026-06-11 当時の記録)。ログ抜粋:
...
無効化のスコープと安全性:
- 対象は E2E ジョブ (`e2e-test`) の matrix `windows_x86_64` のみ (`.github/workflows/ci.yml` の `e2e-test` ジョブ `strategy.matrix.platform` の全 entry で `name:` フィールドが定義されているため `if: ${{ matrix.platform.name == 'windows_x86_64' }}` の条件式で安全に分岐できる)
...
- `.github/workflows/ci.yml` の `e2e-test` ジョブ matrix `windows_x86_64` で、テスト実行ステップ（`Run E2E tests`。`uv run pytest -v -x` を実行）より前に `vEthernet (Default Switch)` を無効化する PowerShell ステップが追加されている
...
- `CHANGES.md` の `## develop` 配下、`### misc` セクションに以下の形式で `[FIX]` エントリを追記する。`### misc` セクション内では凡例順（CHANGE → ADD → UPDATE → FIX）を尊重する。挿入位置は、既存の `[FIX]` エントリがある場合はその先頭（最新）エントリの直前（`### misc` 内の各種別グループでは新しいエントリほど先頭に置く慣例に従う。例: libwebrtc のバージョン更新エントリは常に既存 `[UPDATE]` エントリの先頭に追加されている）、無い場合は `### misc` の末尾（[UPDATE] 群の後）とする。`### misc` 内既存エントリの並べ替えは本 issue のスコープ外。担当者ハンドル `@<担当者>` は PR 作成者のものに書き換える:
...
`.github/workflows/ci.yml` の `e2e-test` ジョブ matrix `windows_x86_64` のテスト実行ステップ（`Run E2E tests`。`uv run pytest -v -x` を実行）の直前に、以下の PowerShell ステップを追加する。対象 NIC を `vEthernet (Default Switch)` の 1 つに限定し、無効化前後の `Get-NetAdapter` 出力をログに残す。
