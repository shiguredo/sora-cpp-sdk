"""Sumomo の Intel VPL E2E テスト

Intel VPL (Video Processing Library) を使用した VP9/AV1/H264/H265 のテスト
"""
...
pytestmark = pytest.mark.skipif(
    ...
    reason="INTEL_VPL not set in environment",
)
...
def test_connection_stats(sora_settings, free_port, video_codec_type):
    """VP9/AV1/H264/H265 での接続時の統計情報を確認（Intel VPL 使用）"""
    expected_mime_type = f"video/{video_codec_type}"

    # エンコーダー設定を準備
    encoder_params = {}
    if video_codec_type == "VP9":
        encoder_params["vp9_encoder"] = "intel_vpl"
    elif video_codec_type == "AV1":
        encoder_params["av1_encoder"] = "intel_vpl"
    elif video_codec_type == "H264":
        encoder_params["h264_encoder"] = "intel_vpl"
    elif video_codec_type == "H265":
        encoder_params["h265_encoder"] = "intel_vpl"
    ...
    with Sumomo(
        ...
        **encoder_params,
        ...
        # エンコーダー実装が libvpl であることを確認
        assert "encoderImplementation" in video_outbound
        assert video_outbound["encoderImplementation"] == "libvpl"
...
def test_simulcast(sora_settings, free_port, video_codec_type, expected_encoder_implementation):
    """サイマルキャスト接続時の統計情報を確認（Intel VPL 使用）"""
    # エンコーダー設定を準備
    encoder_params = {}
    if video_codec_type == "VP9":
        encoder_params["vp9_encoder"] = "intel_vpl"
...
def test_sendonly_recvonly_pair(
    ...
    """sendonly と recvonly のペアを作成して送受信を確認（Intel VPL 使用）"""
    ...
    decoder_params = {}
    if video_codec_type == "VP9":
        decoder_params["vp9_decoder"] = "intel_vpl"
    elif video_codec_type == "AV1":
        decoder_params["av1_decoder"] = "intel_vpl"
    elif video_codec_type == "H264":
        decoder_params["h264_decoder"] = "intel_vpl"
    elif video_codec_type == "H265":
        decoder_params["h265_decoder"] = "intel_vpl"
