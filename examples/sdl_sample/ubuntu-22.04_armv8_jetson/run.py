...
def install_deps(
    ...
    with cd(BASE_DIR):
        ...
        webrtc_info = get_webrtc_info(
            "ubuntu-22.04_armv8", local_webrtc_build_dir, install_dir, debug
        )
        ...
        if local_sora_cpp_sdk_dir is None:
            install_sora_and_deps("ubuntu-22.04_armv8_jetson", source_dir, install_dir)
        else:
            build_sora(
...
def main():
    ...
    with cd(sample_build_dir):
        webrtc_info = get_webrtc_info(
            "ubuntu-22.04_armv8", args.local_webrtc_build_dir, install_dir, args.debug
        )
