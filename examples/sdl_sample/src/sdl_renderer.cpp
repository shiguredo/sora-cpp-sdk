...
SDLRenderer::SDLRenderer(int width, int height, bool fullscreen)
  ...
  // BaseRenderer の描画スレッドを開始する。
  // 枠割りは BaseRenderer が Sink の入力サイズから計算する。
  Start();
}

SDLRenderer::~SDLRenderer() {
  // 描画スレッドを止めてから SDL のリソースを破棄する。
  Stop();
  if (renderer_) {
...
void SDLRenderer::PollEvent() {
  ...
  while (SDL_PollEvent(&e)) {
    if (e.type == SDL_EVENT_WINDOW_RESIZED &&
      ...
      // ウィンドウサイズの変更は BaseRenderer に伝えて枠を再計算させる。
...
void SDLRenderer::Render(uint8_t* image,
  ...
  // RenderThreadStarted() で SDL_CreateRenderer に失敗した場合は
  // 描画先が存在しないため、何も描画せずに次のフレームを待つ。
  if (renderer_ == nullptr) {
    return;
  }

  // BaseRenderer が合成したキャンバス全体を 1 枚のテクスチャとして描画する。
