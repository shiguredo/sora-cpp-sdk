...
def macos_sdl3_args(ctx: BuildContext, _: Any) -> List[str]:
...
def linux_x86_sdl3_args(_: BuildContext, webrtc_info: Any) -> List[str]:
    return [
        f"-DCMAKE_C_COMPILER={os.path.join(webrtc_info.clang_dir, 'bin', 'clang')}",
        f"-DCMAKE_CXX_COMPILER={os.path.join(webrtc_info.clang_dir, 'bin', 'clang++')}",
    ]
...
def linux_rootfs_sdl3_args(ctx: BuildContext, webrtc_info: Any) -> List[str]:
...
def windows_sdl3_args(_: BuildContext, __: Any) -> List[str]:
    return []
...
def raspberry_post_build(ctx: BuildContext, sora_info: Any) -> None:
    sample_build_dir = os.path.join(ctx.build_dir, "sumomo")
    cmd(
        [
            "cp",
            os.path.join(sora_info.sora_install_dir, "lib", "libcamerac.so"),
            os.path.join(sample_build_dir, "libcamerac.so"),
        ]
    )
...
PLATFORMS: Dict[str, PlatformConfig] = {
    "macos_arm64": PlatformConfig(
        ...
        sdl3_platform="macos",
        sdl3_cmake_args=macos_sdl3_args,
    ...
    "ubuntu-22.04_x86_64": PlatformConfig(
        ...
        sdl3_platform="linux",
        sdl3_cmake_args=linux_x86_sdl3_args,
    ...
    "ubuntu-22.04_armv8": PlatformConfig(
        ...
        cmake_bin_components=("cmake", "bin"),
        sdl3_platform="linux",
        sdl3_cmake_args=linux_rootfs_sdl3_args,
    ...
    "raspberry-pi-os_armv8": PlatformConfig(
        ...
        sdl3_cmake_args=linux_rootfs_sdl3_args,
        cmake_toolchain_args=linux_rootfs_cmake_toolchain_args,
        rootfs_conf="raspberry-pi-os_armv8.conf",
        post_build=raspberry_post_build,
    ),
    "windows_x86_64": PlatformConfig(
        ...
        sdl3_cmake_args=windows_sdl3_args,
...
def install_deps(config: PlatformConfig, ctx: BuildContext) -> Any:
    with cd(BASE_DIR):
        ...
        install_sdl3(
            ...
            version_file=os.path.join(ctx.install_dir, "sdl3.version"),
            source_dir=ctx.source_dir,
            build_dir=ctx.build_dir,
            install_dir=ctx.install_dir,
            debug=ctx.debug,
            platform=config.sdl3_platform,
            cmake_args=config.sdl3_cmake_args(ctx, webrtc_info),
        )
...
def configure_and_build(config: PlatformConfig, ctx: BuildContext, configuration: str, webrtc_info: Any):
    ...
    with cd(sample_build_dir):
        ...
        cmake_args = [
            ...
            f"-DSDL3_DIR={cmake_path(os.path.join(ctx.install_dir, 'sdl3'))}",
        ]
        ...
        if config.post_build is not None:
            config.post_build(ctx, sora_info)
