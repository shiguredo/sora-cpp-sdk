...
struct SoraVideoEncoderFactoryConfig {
  ...
  - 一部の kNative なバッファーの実装において、バッファーを複数回読み込めないという制限があり、サイマルキャストで問題になった
    - サイマルキャストは複数ストリームを送信するため、バッファーを複数回読みこむ必要がある
- バッファーを I420 に変換することで複数回読めるようになったが、変換処理によりエンコードの性能が低下した
  ...
  - そのため、 I420 への変換処理の有無を制御するフラグが必要になった
  */
